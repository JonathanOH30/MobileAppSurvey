...
//  Cronos
//
//  Created by Jonathan Ochoa Henriquez on 03/06/22.
//
...
class ViewController: UIViewController {
    ...
    @IBAction func surveyTapped(_ sender: AnyObject) {
        let taskViewController = ORKTaskViewController(task: nivelArch_1, taskRun: nil)
        taskViewController.delegate = self
        present(taskViewController, animated: true, completion: nil)
    }
    
    @IBAction func survey2Tapped(_ sender: AnyObject) {
        let taskViewController = ORKTaskViewController(task: nivelArch_2, taskRun: nil)
        taskViewController.delegate = self
        present(taskViewController, animated: true, completion: nil)
    }
    
    @IBAction func survey3Tapped(_ sender: AnyObject) {
        let taskViewController = ORKTaskViewController(task: nivelArch_3, taskRun: nil)
        taskViewController.delegate = self
        present(taskViewController, animated: true, completion: nil)
    }
        ...
        case .completed:
                ...
                for stepResults in taskResult! as! [ORKStepResult]
                {
                    for result in stepResults.results!
                    {
                        ...
                        if res.identifier == "Question_23"
                        {
                            GlobalValue += (FinalResult(miguel: res.choiceAnswers! as NSArray))
                        }
                        if res.identifier == "Question_24"
                        {
                            GlobalValue += (FinalResult(miguel: res.choiceAnswers! as NSArray))
                        }
